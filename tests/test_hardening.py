...
from codenexus.mcp_server import CodeNexusMCPServer
...
def _run_mcp(server, messages):
    inp = "\n".join(json.dumps(m) if isinstance(m, dict) else m for m in messages) + "\n"
    old_in, old_out = sys.stdin, sys.stdout
    sys.stdin, sys.stdout = io.StringIO(inp), io.StringIO()
    try:
        server.run()
        return [line for line in sys.stdout.getvalue().splitlines() if line.strip()]
    finally:
        sys.stdin, sys.stdout = old_in, old_out


def test_mcp_notifications_get_no_response(temp_dir):
    srv = CodeNexusMCPServer(str(temp_dir))
    out = _run_mcp(
        srv,
        [
            {"jsonrpc": "2.0", "id": 1, "method": "initialize", "params": {}},
            {"jsonrpc": "2.0", "method": "notifications/initialized"},
            {"jsonrpc": "not-a-method"},  # unknown notification: silent
        ],
    )
...
def test_mcp_protocol_negotiation_and_errors(temp_dir):
    srv = CodeNexusMCPServer(str(temp_dir))
    out = _run_mcp(
        srv,
        [
    ...
    assert by_id[2]["error"]["code"] == -32601           # unknown tool
    assert by_id[3]["result"].get("isError") is True     # bad args -> isError result
    assert by_id[None]["error"]["code"] == -32700        # parse error, null id
    assert by_id[5]["error"]["code"] == -32601           # unknown method


def test_mcp_tools_roundtrip_on_real_index(temp_dir):
    ...
    out = _run_mcp(
        ...
        [
            ...
            {"jsonrpc": "2.0", "id": 3, "method": "tools/call",
             "params": {"name": "get_skeleton",
                        "arguments": {"file_path": str(temp_dir / "m.py")}}},
        ],
    )
    responses = [json.loads(line) for line in out]
    tools = responses[0]["result"]["tools"]
    assert {t["name"] for t in tools} >= {"run_pipeline", "get_context_capsule"}

    status = json.loads(responses[1]["result"]["content"][0]["text"])
    assert status["nodes"] >= 1

    skeleton = json.loads(responses[2]["result"]["content"][0]["text"])
    assert any("searchable_thing" in s for s in skeleton["skeletons"])
